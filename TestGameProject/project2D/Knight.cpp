...
Knight::Knight() : MilitaryUnit()
{
	m_eUnitType = Unit::UType::Knight;
	m_fAttackTimer = 3.0f;
...
Knight::Knight(Tile* pTile, Empire* pTeam)
{
	Initialise(pTile, pTeam);
	m_eUnitType = Unit::UType::Knight;
}
...
void Knight::Initialise(Tile* pTile, Empire* pTeam)
{
...
void Knight::Update(float fDeltaTime)
{
	MilitaryUnit::Update(fDeltaTime);
}

void Knight::Draw(aie::Renderer2D* pRenderer)
{
...
void Knight::Upgrade(Icon::IType eUpgrade)
{
...
void Knight::AttackBuilding(float fDeltaTime)
{
...
void Knight::AttackUnit(float fDeltaTime)
{
	...
	if (m_pAttacking)
	{
		...
		if (v2ToUnit.magnitudeSq() < 30 * 30)
		{
			if (m_fAttackTimer >= m_fAttackCooldown)
			{
				...
				//take roughly 2 hits to kill enemy
				float fAttackAmount = 50.0f;
